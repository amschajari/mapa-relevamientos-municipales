...
async function checkDB() {
  const { data, error } = await supabase
    ...
    .select('*')
    // order by id since they're the only stable thing, or I can just filter by barrio
    .eq('barrio_nombre', 'San Clemente')
    .limit(10);
  ...
  data.forEach((row, i) => {
    console.log(`Row ${i}: nombre=${row.nombre}, cableado=${row.cableado}`);
